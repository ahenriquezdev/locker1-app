...
const { generateEncryptionKey, generateIV, encrypt, decrypt } = require('../utils/encryption');
...
router.post('/encryption-key', authMiddleware, async (req, res) => {
    try {
        ...
        const encryptionKey = req.body.key ? Buffer.from(req.body.key, 'base64') : generateEncryptionKey();
